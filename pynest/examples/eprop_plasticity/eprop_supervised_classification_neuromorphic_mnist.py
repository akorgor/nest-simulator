...
image_specs = {
    "n_pixels_x": 34,  # number of pixels in horizontal direction
    "n_pixels_y": 34,  # number of pixels in vertical direction
    "n_pixels_polarity": 2,  # number of pixels in the dimension coding for polarity
}

n_pixels = image_specs["n_pixels_x"] * image_specs["n_pixels_y"] * image_specs["n_pixels_polarity"]

n_in = n_pixels - len(pixels_blocklist)  # number of input neurons
...
def load_image(file_path, image_specs, pixels_blocklist=None):
    n_pixels_x = image_specs["n_pixels_x"]
    n_pixels_y = image_specs["n_pixels_y"]
    n_pixels_polarity = image_specs["n_pixels_polarity"]

    n_pixels = n_pixels_polarity * n_pixels_x * n_pixels_y

    with open(file_path, "rb") as f:
        byte_array = f.read()
    byte_array = np.asarray([x for x in byte_array])
    ...
    times = np.around(times * duration["sequence"] / time_max)

    image = [[] for _ in range(n_pixels)]
    for polarity, x, y, time in zip(polarities, y_coords, x_coords, times):
        pixel = polarity * n_pixels_x * n_pixels_y + x * n_pixels_y + y
        image[pixel].append(time)

    if pixels_blocklist is not None:
        pixels_to_keep = sorted(set(range(n_pixels)) - set(pixels_blocklist))
        image = [image[pixel] for pixel in pixels_to_keep]
    return image


class DataLoader:
    def __init__(self, path, selected_labels, group_size, image_specs, pixels_blocklist=None):
        self.path = path
    ...
    def get_new_evaluation_group(self):
        ...
        images_group = [
            load_image(self.all_sample_paths[i], self.image_specs, self.pixels_blocklist) for i in selected_indices
        ]
...
data_loader_train = DataLoader(train_path, selected_labels, group_size, image_specs, pixels_blocklist)
data_loader_test = DataLoader(test_path, selected_labels, group_size, image_specs, pixels_blocklist)
