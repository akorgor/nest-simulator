...
consists of a recurrent network that receives frozen Poisson noise input from spike generators and projects onto one
readout neuron. The readout neuron compares the network signal :math:`y` with the target signal
...
n_iter_train = cfg["n_iter_train"]  # number of iterations, 2000 in reference [2]
...
if cfg["model_nrn_rec"] in ["eprop_iaf_psc_delta", "eprop_iaf_psc_delta_adapt"]:
...
if cfg["record_dynamics"]:
    ...
    dynamics_recorders = [mm_out, mm_rec, sr_in, sr_rec]
...
if cfg["model_nrn_rec"] == "eprop_iaf":
    weights_inp_rec *= scale_factor
    weights_rec_rec *= scale_factor
    weights_out_rec *= scale_factor
elif cfg["model_nrn_rec"] in ["eprop_iaf_psc_delta", "eprop_iaf_psc_delta_adapt"]:
    weights_out_rec /= scale_factor
...
nest.data_prefix = "0_"
nest.Simulate(duration["total_offset"] + (n_iter_train-1)*duration["sequence"])
...
nest.data_prefix = "1_"
nest.Simulate(duration["sim"] - nest.biological_time)
...
if cfg["record_dynamics"]:
    ...
    tools.save_recordings("multimeter_rec")
    tools.save_recordings("spike_recorder_in")
    tools.save_recordings("spike_recorder_rec")
