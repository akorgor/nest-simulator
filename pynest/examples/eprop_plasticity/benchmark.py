...
if n_in > 0:
    if cfg["input_generator"] == "poisson_generator":
        ...
        sg_params = [dict(spike_times=arr.astype(float)) for arr in times_split]
    ...
    nest.Connect(
        nrns_in,
        nrns_rec,
        dict(rule="fixed_indegree", indegree=cfg["indegree_in"]),
...
nest.Connect(
    nrns_rec, nrns_out, dict(rule="fixed_indegree", indegree=cfg["indegree_out"]), dict(synapse_model="static_synapse")
)
...
if model_conn_fb is not None:
    nest.Connect(
        nrns_out,
        nrns_rec,
        dict(rule="fixed_outdegree", outdegree=cfg["outdegree_fb"]),
        dict(synapse_model=model_conn_fb),
    )
...
if model_gen_rate is not None:
    ...
    nest.Connect(
        gen_rate_target,
        nrns_out,
        dict(rule="one_to_one"),
        dict(synapse_model="rate_connection_delayed", receptor_type=2),
    )
