...
params_nrn_ad = dict(
    ...
    C_m=1.0,
    c_reg=cfg["c_reg"] / duration["sequence"] * duration["learning_window"],
    E_L=0.0,
    eprop_isi_trace_cutoff=cfg["cutoff"],  # cutoff of integration of eprop trace between spikes
    f_target=10.0,
...
eta_train = cfg["eta"] / duration["learning_window"] * scale_factor**2  # learning rate for training phase
                ...
                if do_early_stopping:
                    ...
                    if self.k_iter > 0 and self.error < stop_crit:
                        ...
                        if self.error < stop_crit:
                            break
                self.run_phase("training", eta_train, n_iter_validate_every, evaluate=True)
        else:
            self.run_phase("training", eta_train, n_iter_train, evaluate=True)

        self.run_phase("test", eta_test, n_iter_test, evaluate=True)
...
if cfg["save_weights"] and comm.rank == 0:
