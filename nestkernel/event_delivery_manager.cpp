    ...
    if ( not kernel().connection_manager.use_compressed_spikes() )
    {
      for ( size_t i = 0; i < num_batches; ++i )
      {
        for ( size_t j = 0; j < SPIKES_PER_BATCH; ++j )
        {
          ...
          if ( spike_data.is_pure_activation_marker() )
          {
            se_batch[ j ].set_pure_activation();
          }
          else
          {
            se_batch[ j ].unset_pure_activation();
          }
        }
        for ( size_t j = 0; j < SPIKES_PER_BATCH; ++j )
        {
      ...
      for ( size_t i = 0; i < num_batches; ++i )
      {
        ...
        for ( size_t j = 0; j < SPIKES_PER_BATCH; ++j )
        {
          if ( lcid_batch[ j ] != invalid_lcid )
          {
            ...
            auto pure_activation = se_batch[ j ].get_pure_activation();
