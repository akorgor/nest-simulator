  ...
  const long eprop_isi_trace_cutoff )
{
  ...
  if ( not pure_activation )
  {
    ...
    {
      update_history_.emplace_back( t_current_update + shift, 1 );
    }
  }
...
EpropArchivingNode< HistEntryT >::erase_used_eprop_history( const long t_spike, const long t_spike_previous )
{
  ...
  auto begin_it = eprop_history_.begin() + eprop_head_;
  auto it_end = std::lower_bound( begin_it, eprop_history_.end(), time_end);

  eprop_head_ += std::distance(begin_it, it_end);
